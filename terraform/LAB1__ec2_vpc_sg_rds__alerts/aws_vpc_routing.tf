...
resource "aws_route_table_association" "route_sn_za_pro_pri_20" {
  subnet_id      = module.aws_sn_za_pro_pri_20.id
  route_table_id = module.aws_private_route_table_za.id
}

# Associate private networks in zone B to private route table
resource "aws_route_table_association" "aws_sn_zb_pro_pri_40" {
  subnet_id      = module.aws_sn_zb_pro_pri_40.id
